...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-medical-50 via-white to-clinical-50 overflow-x-hidden">
      {/* Offline Banner */}
      {!isOnline && (
        <div className="bg-warning-500 text-warning-900 px-3 py-2 text-center text-xs sm:text-sm font-medium flex items-center justify-center gap-2 safe-area-top">
          <WifiOff className="w-4 h-4 flex-shrink-0" />
          <span className="truncate">Offline - All calculators work! Data saved locally.</span>
        </div>
      )}

      {/* Header - WHO Standard Medical Blue */}
      <header className="bg-medical-700 text-white shadow-lg safe-area-top">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-base sm:text-lg md:text-2xl lg:text-3xl font-heading font-bold truncate">Clinical Critical Calculator</h1>
                <p className="text-xs sm:text-sm text-medical-100 mt-0.5 hidden sm:block">WHO-Aligned Critical Care Management</p>
              </div>
            </div>
      ...
      {patientInfo.name && (
        <div className="bg-gradient-to-r from-clinical-100 to-clinical-50 border-b-2 border-clinical-300 no-print">
          <div className="container mx-auto px-3 sm:px-4 py-2 sm:py-3">
            <div className="flex items-start justify-between gap-2">
              <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-x-3 gap-y-1 sm:gap-4 md:gap-6 text-xs sm:text-sm min-w-0">
                <div className="truncate"><strong className="font-clinical">Patient:</strong> {patientInfo.name}</div>
                {patientInfo.hospital && <div className="truncate hidden sm:block"><strong className="font-clinical">Hospital:</strong> {patientInfo.hospital}</div>}
                <div><strong className="font-clinical">Age:</strong> <span className="font-mono">{patientInfo.age}y</span></div>
                <div><strong className="font-clinical">Gender:</strong> {patientInfo.gender === 'male' ? 'M' : 'F'}</div>
                <div className="truncate"><strong className="font-clinical">MRN:</strong> <span className="font-mono">{patientInfo.hospitalNumber}</span></div>
                {patientInfo.diagnosis && <div className="truncate col-span-2 sm:col-span-1"><strong className="font-clinical">Dx:</strong> {patientInfo.diagnosis}</div>}
              </div>
              <button
                type="button"
                onClick={() => setShowPatientForm(true)}
                className="text-xs text-medical-600 hover:text-medical-800 underline flex-shrink-0 touch-manipulation min-h-[32px] flex items-center font-medium"
              >
                Edit
              </button>
            </div>
            {patientInfo.comorbidities.length > 0 && (
              <div className="mt-1 sm:mt-2 text-xs line-clamp-2">
                <strong className="font-clinical">Comorbidities:</strong> {patientInfo.comorbidities.join(', ')}
              </div>
            )}
          </div>
        </div>
      ...
      <nav className="bg-white shadow-md sticky top-0 z-10 no-print border-b border-clinical-200">
        <div className="container mx-auto px-0 sm:px-4">
          <div className="flex overflow-x-auto scrollbar-hide -webkit-overflow-scrolling-touch">
            <button
              type="button"
              onClick={() => setShowPatientForm(true)}
              className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-3 sm:py-4 font-medium sm:font-semibold text-medical-600 hover:bg-medical-50 transition-all whitespace-nowrap border-r border-clinical-200 touch-manipulation text-xs sm:text-sm min-h-[48px]"
            >
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 md:px-6 py-3 sm:py-4 font-medium sm:font-semibold transition-all whitespace-nowrap touch-manipulation text-xs sm:text-sm min-h-[48px] ${
                    activeTab === tab.id
                      ? 'text-medical-700 border-b-3 border-medical-600 bg-medical-50'
                      : 'text-clinical-700 hover:text-medical-600 hover:bg-clinical-50 active:bg-clinical-100'
                  }`}
                >
      ...
      {showPatientForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-xl sm:rounded-lg shadow-xl w-full sm:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
            <div className="sticky top-0 bg-medical-700 text-white p-4 sm:p-6 rounded-t-xl sm:rounded-t-lg flex-shrink-0">
              <h2 className="text-lg sm:text-xl md:text-2xl font-heading font-bold">Patient Information</h2>
              <p className="text-xs sm:text-sm text-medical-100 mt-0.5 sm:mt-1">Enter patient details for all calculators</p>
            </div>
            
            <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto max-h-[60vh]">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-clinical-800 mb-1.5 sm:mb-2">
                    Patient Name *
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 sm:px-4 py-2.5 sm:py-2 text-sm sm:text-base border border-clinical-300 rounded-md focus:ring-2 focus:ring-medical-500 focus:border-medical-500 text-clinical-900 touch-manipulation"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 sm:px-4 py-2.5 sm:py-2 text-sm sm:text-base border border-clinical-300 rounded-md focus:ring-2 focus:ring-medical-500 focus:border-medical-500 text-clinical-900 font-mono touch-manipulation"
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-clinical-800 mb-1.5 sm:mb-2">
                  Comorbidities
                  <span className="text-xs text-clinical-500 ml-1 sm:ml-2 block sm:inline">(Select all that apply)</span>
                </label>
                <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 gap-1.5 sm:gap-2 max-h-48 sm:max-h-60 overflow-y-auto border border-clinical-200 rounded-md p-2 sm:p-3">
                  {availableComorbidities.map((comorbidity) => (
                    <label key={comorbidity} className="flex items-center space-x-2 cursor-pointer hover:bg-clinical-50 active:bg-clinical-100 p-1.5 sm:p-1 rounded touch-manipulation min-h-[36px]">
                      <input
                        type="checkbox"
                        checked={patientInfo.comorbidities.includes(comorbidity)}
                        onChange={() => toggleComorbidity(comorbidity)}
                        className="w-5 h-5 sm:w-4 sm:h-4 text-medical-600 border-clinical-300 rounded focus:ring-medical-500"
                      />
                      <span className="text-xs sm:text-sm text-clinical-800">{comorbidity}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="sticky bottom-0 bg-clinical-50 px-4 sm:px-6 py-3 sm:py-4 rounded-b-lg flex gap-2 sm:gap-3 border-t border-clinical-200 safe-area-bottom">
              <button
                type="button"
                onClick={() => setShowPatientForm(false)}
                className="flex-1 px-4 sm:px-6 py-3 sm:py-2 bg-medical-600 text-white text-sm sm:text-base font-semibold rounded-md hover:bg-medical-700 active:bg-medical-800 transition-colors touch-manipulation min-h-[44px]"
              >
                Save & Continue
              </button>
              <button
                onClick={() => setShowPatientForm(false)}
                className="px-4 sm:px-6 py-3 sm:py-2 border border-clinical-300 text-clinical-800 text-sm sm:text-base font-semibold rounded-md hover:bg-clinical-50 active:bg-clinical-100 transition-colors touch-manipulation min-h-[44px]"
              >
                Cancel
              </button>
            </div>
      ...
      {/* Footer - WHO Standard */}
      <footer className="bg-clinical-900 text-white py-4 sm:py-6 mt-8 sm:mt-16 no-print safe-area-bottom">
        <div className="container mx-auto px-3 sm:px-4 text-center">
          <p className="text-sm sm:text-lg font-heading font-semibold text-medical-400 mb-2 sm:mb-3">
            POWERED BY BONNESANTE MEDICALS
          </p>
          <p className="text-xs sm:text-sm italic text-clinical-300 mb-3 sm:mb-4">
            Caring for and healing with passion
          </p>
          <p className="text-xs sm:text-sm text-clinical-400">
            Clinical Critical Calculator - For healthcare professionals only
          </p>
          <p className="text-xs text-clinical-500 mt-1 sm:mt-2 px-2">
